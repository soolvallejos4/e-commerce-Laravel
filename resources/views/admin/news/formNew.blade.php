...
@section('main')
    <h2 class="my-4">Publicar una Noticia</h2>

    <form action="{{ route('admin.news.processNew') }}" method="post" enctype="multipart/form-data">
        @csrf
        <div class="mb-3">
            <label for="title" class="form-label">Título</label> <br> <span>El título debe tener al menos 3
                caracteres.</span>
        ...
        <div class="mb-3">
            ...
            <input type="text" id="author" name="author" class="form-control" value="{{ auth()->user()->email }}" readonly>
        </div>
        <div class="mb-3">
            <label for="news_date" class="form-label">Fecha Publicacion</label>
            <input type="text" id="news_date" name="news_date" class="form-control" value="{{ old('news_date', now()->format('d-m-Y H:i')) }}" readonly>
            @error('news_date')
